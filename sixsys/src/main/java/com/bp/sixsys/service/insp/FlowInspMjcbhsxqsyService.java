...
import com.bp.sixsys.mapper.insp.FlowInspMjcbhsxqsyMapper;
 ...
 * @author pengwanli
 * @version 1.0
 * @Description: 民警操办婚丧喜庆事宜报告单服务层
 * @date 2019年12月02日
*/
@Service
public class FlowInspMjcbhsxqsyService extends BaseServiceImplByAct<FlowInspMjcbhsxqsyMapper,FlowInspMjcbhsxqsy> {

	@Resource
	private FlowInspMjcbhsxqsyMapper flowInspMjcbhsxqsyMapper;
	...
	@Override
	public BaseMapperUUID<FlowInspMjcbhsxqsy> getMapper() {
        request.setAttribute("sysType","insp");
        request.setAttribute("tableName","flow_insp_mjcbhsxqsy");
		return flowInspMjcbhsxqsyMapper;
	}

    @Override
    @Transactional(rollbackFor = Exception.class)
    @LcnTransaction
    public String insert(FlowInspMjcbhsxqsy entity) {
    ...
    @Transactional(rollbackFor = Exception.class)
    @LcnTransaction
    public void update(FlowInspMjcbhsxqsy entity) {
        entity.setFlowState(0);
        entity.setFlowSn(sysClient.getNumberFlow("ACTIVITI_SN").msg);
        entity.setDocState("新建");
        super.update(entity);
    ...
    public ReturnBean submit(String uuids) {
        ...
        //政工与纪检部门审核
        paraMap.put("zgyjjbmId", sysClient.findFlowRoleByCode("044").data.getUserIds() + ",1");
        //事后报告
        paraMap.put("shbgId", UserUtils.getCurrentUser().getId() + ",1");

        paraMap.put("baseUrl", "sixsys/flowInspMjcbhsxqsy");
        for (String id :
                uuids.split(",")) {
            FlowInspMjcbhsxqsy flowInspMjcbhsxqsy = getMapper().selectById(id);
            paraMap.put("tableName","flow_insp_mjcbhsxqsy");
            paraMap.put("sysType","insp");
            this.startProcess(id, "flowInspMjcbhsxqsy", paraMap);
            ...
            para.put("pId", pid);
            para.put("pKey", "flowInspMjcbhsxqsy");
            para.put("msg", "提交");
